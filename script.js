...
function applySiteConfig() {
  ...
  if (topBarText) topBarText.textContent = config.topBarText || 'MB Style • Produits Digitaux';

  const heroTitle = document.getElementById('heroTitle');
  if (heroTitle) heroTitle.textContent = config.heroTitle || 'MB Style Produits Digitaux';
  ...
  if (productsHeading) productsHeading.textContent = config.productsHeading || 'Produits numériques à vendre';

  const shopTitle = document.getElementById('shopTitle');
  if (shopTitle) shopTitle.textContent = config.shopTitle || 'Catégories Produits Digitaux';
  ...
  if (contactTitle) contactTitle.textContent = config.contactTitle || 'Contactez-nous pour vos offres digitales';
  ...
  if (newsletterTitle) newsletterTitle.textContent = config.newsletterTitle || 'Offres digitales par email';
}
...
function showCheckout() {
  ...
  summary.innerHTML = `
    <div class="border border-gray-200 p-4 bg-gray-50">
      ...
      <p class="text-sm font-bold mt-3">Total : ${total} €</p>
    </div>
  `;
}
...
function submitOrder(event) {
  ...
  const mailtoLink = `mailto:hello@mb-style.com?subject=Nouvelle%20commande%20MB%20Style&body=${encodeURIComponent(orderText)}`;
